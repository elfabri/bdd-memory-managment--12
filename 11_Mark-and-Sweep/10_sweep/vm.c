/* Every object now has an is_marked field
 * that we can use to determine if an object
 * is reachable or not. All we need to do
 * is iterate over all the objects in the VM
 * and free any object that is not marked.
 * Once it's freed, we can also remove it from our VM.
 */
#include "vm.h"
...
void sweep(vm_t *vm) {
  // reset objects marked true to false
  // free objects marked false
