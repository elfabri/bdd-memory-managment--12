/* Our virtual machine needs to track
 * every Snek object that gets created.
 *
 * We are no longer going to track how many
 * times an object is referenced, but instead
 * check at garbage collection time if each object
 * is still referenced at all.
