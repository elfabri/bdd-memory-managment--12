/* boot.dev is using something else where they defined
 * what munit_case is and how it works, in order to 
 * make a diffrence when you run the code on the web, and when
 * you submit it, I installed munit but I don't have the definitions
 * of the things used here, which are not included on munit.
 * I ran the test on the web and it worked, first try.
 */
#include "munit.h"
